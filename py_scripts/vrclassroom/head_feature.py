...
def main(hmd_datas, CaseIds):
	""" Calculate features for incoming test cases.

	in: minimal hmd_data:[['PosX', 'PosY', 'PosZ', 'RotX', 'RotY', 'RotZ']]

	out:[['BlockNum', 'PathLen', 'TimeActive', 'NumRot', 'TotalDeg', 'PercentageDistracted', 'IsMoveRangeLarge', 'CaseId']]
	"""
	new_hmd_datas=[s for s in hmd_datas]
	datas=[]
	for caseId in CaseIds:
		new_hmd_data=[ x for x in new_hmd_datas if x['CaseId']==caseId]
		data=[[s['PosX'],s['PosY'],s['PosZ'],s['RotX'],s['RotY'],s['RotZ']] for s in new_hmd_data]
		datas.append(data)
		
	# compute features
	features = process_feature(CaseIds, datas)
